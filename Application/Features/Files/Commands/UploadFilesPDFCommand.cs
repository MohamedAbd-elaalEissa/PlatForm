...
{
    public record UploadFilesPDFCommand(IFormFile file, int userId, int teacherID,bool isAnswer,int? fileID,int AcademicLevelID) :IRequest<CommonResult>;
    ...
    public class UploadFilesPDFCommandHandler : IRequestHandler<UploadFilesPDFCommand, CommonResult>
    {
        ...
        {
            return await _repository.UploadFilePDF(request.file,request.userId,request.teacherID,request.isAnswer,request.fileID,request.AcademicLevelID);
        }
    }
